...
{
  std::cout << "key_source,build_key_count,build_key_bytes,probe_source,probe_key_count,probe_key_bytes,output_bytes,slot_bytes,requested_load_factor,"
               "capacity,size,actual_load_factor,h2d_keys_ms,insert_ms,build_ms,"
...
void print_human_summary(result_row const& result)
{
  ...
  std::cout << std::fixed << std::setprecision(3)
            << "Set: requested load factor=" << result.requested_load_factor
...
int main(int argc, char** argv)
{
  try {
    ...
    thrust::device_vector<key_type> d_output(probe_key_count);
    ...
    result.vram_total_bytes = free_before_buffers - free_after_set;
