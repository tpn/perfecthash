...
- `--assign-geometry` selects how assignment work is partitioned within the CUDA block:
  - default: `thread`
  - allowed values: `thread`, `warp`, `block`
- `--device-serial-peel-geometry` selects how the `device-serial` peel stage is partitioned:
  - default: `thread`
  - allowed values: `thread`, `warp`, `block`
...
- Stage 1 geometry selection changes execution geometry only.
  It does not introduce shared-memory staging, CUB primitives, or any other memory-model changes yet.
