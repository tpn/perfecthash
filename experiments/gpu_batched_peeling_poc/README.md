...
- `--assign-geometry` is a stage-1 configuration/reporting field for assignment geometry:
  - default: `thread`
  - allowed values: `thread`, `warp`, `block`
- `--device-serial-peel-geometry` is a stage-1 configuration/reporting field for the `device-serial` peel path:
  - default: `thread`
  - allowed values: `thread`, `warp`, `block`
...
- Stage 1 geometry selection records the intended execution geometry only.
  It does not introduce active warp/block execution changes, shared-memory staging, CUB primitives, or any other memory-model changes yet.
