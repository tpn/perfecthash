...
- `--output-format json` emits a single machine-readable JSON object and suppresses the human summary on stdout.
  ...
  - `cpu_ms`
  - `gpu_ns_per_attempt`
  - `gpu_ns_per_attempted_key`
  - `cpu_ns_per_attempt`
  - `cpu_ns_per_attempted_key`
...
- The human summary prints the same normalized timing fields next to the raw `gpu_ms` and `cpu_ms` totals.
- The normalized fields are derived as:
  - `*_ns_per_attempt = time_ms * 1e6 / actual_attempts_tried`
  - `*_ns_per_attempted_key = time_ms * 1e6 / (actual_attempts_tried * actual_keys)`
