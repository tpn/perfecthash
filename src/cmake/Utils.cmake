...
# PerfectHash Customization

# Source: https://stackoverflow.com/a/48900730
# compile options
...
function(add_compile_options_not_config CONFIG)
    foreach(opt ${ARGN})
        add_compile_options("$<$<NOT:$<CONFIG:${CONFIG}>>:${opt}>")
    endforeach()
endfunction()

# compile definitions
function(add_compile_definitions_config CONFIG)
    foreach(opt ${ARGN})
        add_compile_definitions("$<$<CONFIG:${CONFIG}>:${opt}>")
    endforeach()
endfunction()

function(add_compile_definitions_not_config CONFIG)
    foreach(opt ${ARGN})
        add_compile_definitions("$<$<NOT:$<CONFIG:${CONFIG}>>:${opt}>")
    endforeach()
endfunction()

# link options
function(add_link_options_config CONFIG)
    foreach(opt ${ARGN})
        add_link_options("$<$<CONFIG:${CONFIG}>:${opt}>")
    endforeach()
endfunction()

function(add_link_options_not_config CONFIG)
    foreach(opt ${ARGN})
        add_link_options("$<$<NOT:$<CONFIG:${CONFIG}>>:${opt}>")
    endforeach()
endfunction()
