...
--*/
{
    ...
    FOR_EACH_1D(Vertex, NumberOfVertices) {

        if (IsEmpty(Graph->First[Vertex])) {

            //
            // TODO: either make this an atomic increment or separate out into a
            // separate temporary counter array that can be reduced later.
            //
        ...
        if (!IsVisitedVertex(Graph, Vertex)) {
            ...
            // on as part of assignment.  Sanity check the initial assignment
            // value matches, then traverse the graph at this vertex.
            //

            Graph->Assigned[Vertex] == INITIAL_ASSIGNMENT_VALUE;
            GraphCuTraverseRecursive(Graph, Vertex);
        }
    }
