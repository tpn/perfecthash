...
Abstract:
    ...
    Import of bitmap.c from ReactOS.
...
unsigned char BitScanForward64(ULONG64 * Index, ULONG64 Mask)
{
    *Index = 0;
    while (Mask && ((Mask & 1) == 0))
    {
...
unsigned char BitScanReverse64(ULONG64 * const Index, ULONG64 Mask)
{
    *Index = 0;
    while (Mask && ((Mask & (1ULL << 63)) == 0))
    {
        Mask <<= 1;
        ++(*Index);
    }
...
RtlFindLongestRunClear(
    _In_ PRTL_BITMAP BitMapHeader,
    _In_ PBITMAP_INDEX StartingIndex)
{
