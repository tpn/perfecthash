    ...
    )
{
    ...
    OUTPUT_INT((ULONG)TableInfo->NumberOfKeys.QuadPart);
    OUTPUT_RAW(";\n");
    OUTPUT_RAW("inline constexpr size_t number_of_table_elements = ");
    OUTPUT_INT((ULONG)TableInfo->NumberOfTableElements.QuadPart);
    OUTPUT_RAW(";\n\n");
...
EmitKernels:
    if ((Flags & PH_ONLINE_JIT_CUDA_SOURCE_FLAG_OMIT_KERNELS) == 0) {
        if ((Flags & PH_ONLINE_JIT_CUDA_SOURCE_FLAG_OMIT_TABLE_DATA) != 0) {
            OUTPUT_RAW("__global__ void index_kernel(const original_key_type* query_keys, const table_data_type* table_data, uint32_t* out, size_t count) {\n");
        } else {
            OUTPUT_RAW("__global__ void index_kernel(const original_key_type* query_keys, uint32_t* out, size_t count) {\n");
        }
